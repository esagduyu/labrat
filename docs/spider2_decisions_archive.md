# Spider2-DBT & M1–M32 Decisions Archive

> Archived from decisions.md on 2026-05-25. Spider2-DBT has been replaced by ADE-bench.
> Per-milestone build notes are preserved here for historical reference.

---

## Gotchas encountered — Spider2-DBT autoresearch (2026-05-23)
...
extracted for debugging but not validated.
...
### UX recommendations from Spider2 work (future TUI impact)

- **M-Schema in context engine**: serialize dbt catalog as M-Schema and inject into `ContextBundle.dbt_models`. Priority: medium.
- **Compiled SQL in schema explorer**: "View SQL" action (key `s`) opening `ModalScreen` with `compiled_sql`. Priority: low.
- **Snapshot/verifier as TUI validation mode**: expose deterministic verifier as "Validate last query". Priority: low, deferred.

---

## Per-milestone build notes (M1–M32)
