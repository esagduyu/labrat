...
| `LABRAT_MAZE_DIR` | directory path | Optional, not server-specific — read directly by `labrat.maze.store.MazeStore.from_env` (defaults to `cwd`) wherever a tool touches Scent (`search_reference_docs`, the Cartographer pre-pass). Not generated by `print_config` today; add it to the host config's `env` block by hand if you want the served process to read/write Scent somewhere other than its working directory. |
