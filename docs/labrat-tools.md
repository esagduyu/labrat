...
| `LABRAT_MCP_LOG_DIR` | directory path | Optional. When set, every tool dispatch is appended to `<dir>/mcp_tool_calls.jsonl` (tool name, input, ok/error, output, latency) — the same audit-trace format `claude-mcp`'s DAB driver uses (`append_tool_trace`). Unset = no logging. `get_artifact` dispatches (see below) are logged through this same path when the ledger is on. |
...
| `LABRAT_MCP_LEDGER` | `"1"` to enable | Optional, opt-in, **default off**. Turns on a server-side Context Ledger: any tool result whose serialized payload exceeds the ledger's byte budget is bounded to a `[context ledger] ...` preview block (summary + capped preview + `artifact_ref: ...` line) instead of being sent to the host in full; the untruncated payload is stashed in the result store for later retrieval via `get_artifact`. With the flag unset (the default), `_list_tools`/`_call_tool` are **byte-identical** to today — nothing changes unless you opt in. |
| `LABRAT_MCP_RESULT_STORE_DIR` | directory path | Required alongside `LABRAT_MCP_LEDGER=1` (no-op otherwise). Backing directory for the `ResultStore` that holds full, unbounded tool payloads bounded out of context by the ledger. `get_artifact` reads back from here. |
...
## Opt-in server-side Context Ledger + `get_artifact`

Set `LABRAT_MCP_LEDGER=1` (plus `LABRAT_MCP_RESULT_STORE_DIR`, see the env-var table above) to
turn on a server-side Context Ledger — the same bounding idea `AgentLoop`'s in-process ledger
uses, ported to the MCP path where there is no host-side agent loop to do it. **Default off**;
with the flag unset, tool listing and dispatch are byte-identical to the no-ledger path.

With the ledger on, any tool result whose serialized payload exceeds the ledger's byte budget
is no longer sent to the host in full — it's replaced by a bounded `[context ledger] ...`
preview (a short summary, a capped preview of the payload, and an `artifact_ref: ...` line)
while the untruncated payload is stashed in the result store. A synthetic `get_artifact` tool
is added to the served tool list **only when the ledger is on** — it takes a stored `ref` (plus
an optional integer `offset` for paging) and returns up to 64,000 bytes of the full payload
verbatim, well beyond the inline preview's budget; pass a larger `offset` to page through
payloads longer than that. `get_artifact` calls are logged to `LABRAT_MCP_LOG_DIR` the same way
as any other tool dispatch.
