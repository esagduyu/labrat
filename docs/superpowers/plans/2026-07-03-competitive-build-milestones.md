...
**TUI-integration phase (2026-07-06, PM): that follow-on — and the whole M0–M5 → product-surface gap — is now fully specced + planned.** The TUI chat path bypasses `run_agent_task` (hand-rolled 12-tool registry; map: `docs/tui-integration-handoff.md`), so almost nothing above is user-visible. A dedicated Fable planning session produced the umbrella spec `docs/superpowers/specs/2026-07-06-tui-integration-design.md` (8 locked decisions, commit `196b9a3`) + four executable plans (commit `fe2ac75`), each a branch with TDD tasks and code-verified signatures:
- **TUI-M1** `…/plans/2026-07-06-tui-m1-agent-stack.md` — chat through the real agent stack via a shared `agent/session.py` factory (full registry + Context Ledger + `llm_fn` + optional verifier), `Profile` settings fields + SettingsScreen, provider resolution ("auto" → Anthropic w/ key, else claude CLI + degraded warning). Surfaces M3 `explain_lineage`, M4 ledger/`llm_extract`/`run_program` in the TUI.
- **TUI-M2** `…-tui-m2-cartographer.md` — T2c first-connect Cartographer (deterministic-only) + sidecar-fingerprint staleness, detect-and-offer refresh.
- **TUI-M3** `…-tui-m3-harvest.md` — **the M5 follow-on**: CorrectionBuffer capture, extractors stamp `table_scope`, domain-keyed drafts, HarvestReviewScreen, manual + thread-switch triggers, `Profile.harvest_opt_in` (fail-closed). Resolves all three blockers from the M5 Fable review.
...
**Update (2026-07-10, latest — Maze ladder complete):** the same run continued past this doc's M-scope into the Maze-ladder arc — **Cheese v1** (`6f9c259`, the Pillar 2 share surface: provenance-stamped Cheese artifacts), **Trail v1** (`0978018`, "Scent for procedures": intent-retrieved analysis SOPs + the `search_trails` tool), and **Map** (`1b64953`) — **the Warren/Cookbook aggregation layer, now named Map** (the "Warren" name is retired): per-domain bundles of *pointers* to Scent + Trail docs + suggested prompts (`maze/map.py`, soft-miss resolution), auto-seeded from dbt marts structure by the Cartographer (`draft_maps_from_dbt`), and activated as an additive benchmark-safe retrieval filter (`ToolContext.active_maps`; unset = behavior unchanged). With Map, **the moat's Scent → Trail → Map stack is complete** (spec: `docs/superpowers/specs/2026-07-10-map-domain-bundles-design.md`). Still open from this doc: unchanged — extra 2.4 + the DAB `enable_ledger` A/B.
