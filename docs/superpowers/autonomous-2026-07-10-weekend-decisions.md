...
- **Warren design-exploration (RFC) produced** — `docs/superpowers/specs/2026-07-10-warren-design-exploration.md`. NOT a locked spec: Warren's core shape (W1 reference-vs-copy / W2 manual-vs-derived / W3 activation-vs-share-vs-onboarding) is a product-vision call. Recommends v1 = reference + manual + activation-as-retrieval-filter (benchmark-safe, reuses the reserved `warren` kind, no dep/LLM). Blocked on user ratification of W1-W3 + naming + scope-priority + dbt-mapping. This is the terminal autonomous artifact — remaining work all needs user input.
