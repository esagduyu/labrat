...
- **D2 — Capture is EXPLICIT, not LLM-guessed, and PERSISTED IMMEDIATELY.** LLM-extracting "decisions" from chat has a high false-positive rate (most chat isn't a decision) and would add an LLM call + cost + contamination surface. v1 captures decisions explicitly: a `ctrl+shift+d` action opens a one-field `RecordDecisionScreen`; the analyst types the decision; on save it is stored verbatim as `Memory(kind=explicit_user_rule, text=<typed>, table_scope=resolve_table_scope(_last_sql, known_tables), scope=global_)` via `MemoryStore.append` **immediately** (a deliberately-typed decision must survive a quit before the next harvest, unlike an ephemeral chat correction — and immediate persistence also makes it recallable by the existing `recall_memories` tool the same session). **No LLM** — the analyst's words are the decision. `table_scope` is auto-attributed from the last executed SQL when it resolves to a single known table, else global.
- **D3 — Promotion mirrors corrections exactly, minus re-approval noise.** `maze/harvest.py` gains `draft_decision_sections(clusters) -> dict[str, list[Section]]` — heading `## Decisions`, `source="harvested"`, one bullet per decision, contamination-audited fail-loud (identical structure to `draft_harvested_sections`). A private `_cluster_by_scope(memories)` is extracted; `cluster_corrections` (correction kinds) and a new `cluster_decisions` (`explicit_user_rule`) become thin kind-filtered wrappers over it (DRY). **Because decisions persist immediately (D2), the review must show only UNPROMOTED decisions** — the controller filters out any decision whose text already exists as a bullet in the target domain's project-layer `## Decisions` section (reusing the same body-dedup `apply_approved_sections` already does), so the analyst never re-approves a decision they already promoted. Decisions are drafted **alongside** Gotchas in the same `HarvestReviewScreen` (each draft shows its heading), approved/skipped independently, and written via the existing `apply_approved_sections` (which dedups again at write — belt and suspenders).
