...
- **D3 — Trail doc schema = the EXISTING `ScentDoc`, zero model changes** (refined while grounding the plan — most faithful to non-negotiable 5). A Trail is a `kind="trail"` doc where **`domain` holds the intent-slug** (e.g. `compute-retention`; file lands at `trail/compute-retention.md`) and `tables` holds the referenced tables. The retrieval scorer already folds `doc.domain` into heading matching, so the intent-slug contributes to ranking for free. Everything else lives in **sections** (each with `**Source:**` + freshness meta): **When to use · Steps · Reference SQL · Validations · Gotchas**. "When to use" is the human-readable intent phrase + the intent-match surface (parallels Scent's "Quick Reference" — prepended to every hit, excluded from body scoring). `applies_validations` is the **Validations section body** (a bullet list of applicable rule names), not new frontmatter — v1 is read-as-guidance, so the agent reads it; structured validation frontmatter is a v2 concern (auto-apply). No `intent`/`title`/`applies_validations` fields are added to `ScentDoc`.
...
### 3.1 `maze/trail.py` (new — draft + promote, mirrors `maze/harvest.py`)
- `intent_slug(question: str) -> str` — deterministic slug of the question (lowercased, non-alphanumerics→`-`, trimmed) used as `domain`.
- `draft_trail_from_finding(finding, *, all_validations, generated_at, model_id, schema_hash, git_sha) -> ScentDoc` — extracts the Finding's tables (sqlglot, reusing `resolve_table_scope`-style extraction), selects the `all_validations` whose `table_scope` matches those tables, builds a `kind="trail"` `ScentDoc` (`domain=intent_slug(finding.question)`, `tables=<referenced>`) with the five sections (When to use / Steps / Reference SQL / Validations / Gotchas), each stamped `source` per D7 + `schema_hash`/`generated_at`/`model_id`; contamination-audited (`audit_scent_doc`, fail-loud) before returning.
- `apply_trail(store, doc, *, git_root) -> None` — audits + writes via the `apply_approved_sections` invariants adapted for whole-doc Trail promotion: `store.write_doc(doc, kind="trail")`, project-layer, dedup-by-body, git-sha stamped, fail-loud audit BEFORE write.
