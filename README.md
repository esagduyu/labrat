...
> [!NOTE]
> Status: feature-complete v0 alpha. 567 tests passing. End-to-end demo operational against DuckDB. Evaluated on [ADE-bench](https://github.com/dbt-labs/ade-bench) (dbt Labs): **100% easy · 80% medium · 60% hard · 80% overall** (48/60 tasks, DuckDB+dbt, claude-sonnet-4-6, best-of-3) — **82% on the 39-task subset that overlaps with Altimate Code's published DuckDB results (vs. their 77%)**. **Listed on the [DataAgentBench](https://ucbepic.github.io/DataAgentBench/) (UC Berkeley) public leaderboard at 51.4%** (rank #10 of 18; pass@5, claude-sonnet-4-6, LabRat tools via MCP), independently re-validated by the DAB maintainers (see below). Strongest single-dataset signal: **crmarenapro 82%** on a 6-database hybrid query set. Full write-up: [docs/dab-progress-report.md](docs/dab-progress-report.md).
>
> **Leaderboard acceptance & contamination resolution:** our initial submission reported 58.0%, but a trace audit prompted by the DAB maintainers found that our `claude-mcp` harness left the agent with unrestricted Bash/web access, so on some queries it read the benchmark's `ground_truth.csv`/`validate.py` off disk or pulled external labels (`load_dataset("ag_news")`). We disclosed the leak and the maintainers **independently re-validated all 270 answers**, counting every contaminated trial as a non-pass (0-penalty) — their audit found **3 subagent-delegated leaks beyond the 18 we flagged** (21 total). The final accepted **stratified Pass@1 is 51.4%**, and **LabRat is now on the public leaderboard**. The sandbox gate that prevents this class of leak (MCP-only tools, isolated cwd, a contamination backstop, audit-grade tool logging) is implemented, and our own clean full re-run independently lands at ~50% — confirming the figure. Note: agnews is intrinsically unreliable here — even fully sandboxed, the model can recall the public AG News id→label mapping from training, so we caveat agnews rather than treat it as a capability score. The same `load_dataset` leak hit the leaderboard's top entry too ([PR #53](https://github.com/ucbepic/DataAgentBench/pull/53)), who sandboxed and were re-accepted — so it's model-agnostic.
...
| **Phase 5 (full DAB)** | **12 (all official)** | **54** | **51.4%** (on leaderboard; submitted 58.0%, corrected) | **pass@5, claude-mcp driver, full Phase 2+3 substrate. Maintainer-re-validated; rank #10 of 18 — see resolution note below** |

**Phase 5 resolution — accepted onto the leaderboard at 51.4%:** the submitted 58.0% was inflated by a harness flaw — the `claude-mcp` driver left the agent with unrestricted Bash/web access, so on some queries it read `ground_truth.csv`/`validate.py` off disk or loaded external labels. We disclosed it; the DAB maintainers **independently re-validated all 270 answers**, counting every contaminated trial as a non-pass — their audit found **3 subagent-delegated leaks beyond the 18 we flagged** (21 total) — and **added LabRat to the public leaderboard at a stratified Pass@1 of 51.4%** (rank #10 of 18, just below Claude Opus 4.6 ReAct at 54.7%). Our own sandboxed clean re-run (MCP-only `--allowedTools`, isolated cwd, contamination backstop, tool-call logging) independently lands at ~50%, confirming the figure: deps_dev_v1 30% (was 10%), crmarenapro 80%, stockindex 87%, stockmarket 84%, github_repos/patents flat. Caveat: agnews leaks via model *memory* even when sandboxed (the model recalls the public id→label mapping) — same class of issue that hit Altimate ([PR #53](https://github.com/ucbepic/DataAgentBench/pull/53), GPT‑5.5, re-accepted after sandboxing), so it's model-agnostic, not a LabRat weakness. Details: [docs/dab-progress-report.md](docs/dab-progress-report.md).
