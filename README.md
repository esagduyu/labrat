...
> [!NOTE]
> Status: feature-complete v0 alpha. 542 tests passing. End-to-end demo operational against DuckDB. Evaluated on [ADE-bench](https://github.com/dbt-labs/ade-bench) (dbt Labs): **100% easy · 80% medium · 60% hard · 80% overall** (48/60 tasks, DuckDB+dbt, claude-sonnet-4-6, best-of-3) — **82% on the 39-task subset that overlaps with Altimate Code's published DuckDB results (vs. their 77%)**. Evaluated on the **full [DataAgentBench](https://ucbepic.github.io/DataAgentBench/) (UC Berkeley)** 54-query / 12-dataset benchmark: **50.5%** (pass@5, claude-sonnet-4-6, LabRat tools via MCP), corrected after a self-audit (see below). Strongest single-dataset signal: **crmarenapro 82%** on a 6-database hybrid query set. Full write-up: [docs/dab-progress-report.md](docs/dab-progress-report.md).
>
> **Correction (2026-06-03):** the initial submission reported 58.0%, but a trace audit prompted by the DAB maintainers found that our `claude-mcp` harness left the agent with unrestricted Bash/web access, so on some queries it read the benchmark's `ground_truth.csv`/`validate.py` off disk or pulled external labels (`load_dataset("ag_news")`). 18 contaminated trials (16 of 20 agnews, plus one bookreview and one yelp) are **withdrawn**; the remaining trials stand at **50.5%**, with agnews effectively unscored. The sandbox gate that prevents this (MCP-only tools, isolated cwd, a contamination backstop, audit-grade tool logging) is implemented, and a clean full re-run is in progress. Note: agnews is intrinsically unreliable here — even fully sandboxed, the model can recall the public AG News id→label mapping from training, so we caveat agnews rather than treat it as a capability score. The same `load_dataset` leak hit the leaderboard's top entry too ([PR #53](https://github.com/ucbepic/DataAgentBench/pull/53)), who sandboxed and were re-accepted.
...
| **Phase 5 (full DAB)** | **12 (all official)** | **54** | **50.5%** (corrected; submitted 58.0%) | **pass@5, claude-mcp driver, full Phase 2+3 substrate. 18 contaminated trials withdrawn after self-audit — see correction note below** |

**Phase 5 correction (2026-06-03):** the submitted 58.0% was inflated by a harness flaw — the `claude-mcp` driver left the agent with unrestricted Bash/web access, so on some queries it read `ground_truth.csv`/`validate.py` off disk or loaded external labels. Withdrawing the 18 contaminated trials (16 of 20 agnews; one bookreview; one yelp) gives **50.5%**, with agnews effectively unscored (95% → 15%). The other nine datasets show no such access. The sandbox gate (MCP-only `--allowedTools`, isolated cwd, contamination backstop, tool-call logging) is implemented and a clean full re-run is in progress, self-healing across Max-plan session limits. Interim: sandbox holds (MCP-only, no file/web leak); deps_dev_v1 ~30% (was 10%), github_repos/patents flat, agnews honest ~29%. Caveat: agnews leaks via model *memory* even when sandboxed (the model recalls the public id→label mapping) — same class of issue that hit Altimate ([PR #53](https://github.com/ucbepic/DataAgentBench/pull/53), GPT‑5.5, re-accepted after sandboxing), so it's model-agnostic, not a LabRat weakness. Details: [docs/dab-progress-report.md](docs/dab-progress-report.md).

**Phase 5 per-dataset highlights (clean datasets):** crmarenapro **82%** on 6 databases (substrate's strongest single signal); stockindex **100%**, stockmarket 80%; pancancer_atlas 67%; github_repos 50%, googlelocal 50%; deps_dev_v1 10% (stochastic regression from Phase 4); music_brainz_20k 7% and patents 0% (Sonnet ceiling, not infra). bookreview 87% and yelp 60% after withdrawing one contaminated trial each. Three drivers coexist (`raw-bash` for baseline reproducibility, `labrat-agent` for multi-provider standalone use, `claude-mcp` for the Max-plan tool path). Full write-up: [docs/dab-progress-report.md](docs/dab-progress-report.md).
...
- **DataAgentBench clean re-run (top priority)**: re-run the full 54-query benchmark with the agent sandboxed — tool access restricted to the MCP server (no Bash/WebFetch/subagents via `--allowedTools`), benchmark repo off the agent's filesystem, no network egress — so answer-key/external-label access is impossible by construction. This produces a defensible replacement for the corrected 50.5%. Then **Phase 6** (closing the gap to MinusX 63.1%): force-query prompt rule to recover music_brainz_20k (7% → likely ≥40% — the agent has tools and currently chooses not to use them); investigate patents 0% (Sonnet ceiling vs. prompt structure); raise pass@5 to pass@10 to tighten dataset-mean variance (deps_dev_v1 regressed from Phase 4's 40% to 10% — likely n=5 noise)
