...
[DataAgentBench](https://ucbepic.github.io/DataAgentBench/) — 12 datasets / 54 queries / 4 DBMSes (DuckDB, SQLite, Postgres, Mongo). **LabRat is on the leaderboard at stratified Pass@1 74.18%** (accepted entry "GPT-5.6-Luna-Max + Cartographer", PR #72, 2026-07-16, labrat-agent driver + codex provider + Cartographer/levers/hints/ledger). Rank drifts as the board grows (was #5 of 27 at acceptance; #6 after Sentinel/Actioneer's 84.50% displaced the top) — the **durable claim is #2 among UNTUNED-prompt entries** (0.15pp behind Spacedock; every entry above us carries the board's "Tuned prompt ✓" mark). Prior entries stay on the board: Sonnet 4.6 + Cartographer 60.88% (PR #65), first Sonnet 51.38%. **Cite 74.18% as our current number; never 58.0% (contaminated) or 50.5% (interim recompute).** DAB scores are versioned against **current upstream ground truth** (the board re-scores all rows when GTs change). Screenshot: `docs/images/dab-leaderboard-2026-07-16.png`.
...
**Gap-closing levers — UNMERGED on `feat/dab-gap-levers` (validate before merge):** 3 new `_dab_lever_lines` (free-text match/parse completeness, byte-verbatim value delivery, convention-pinning) targeting the 3 gap themes. Smoke n=3 A/B vs the run baseline: **4/5 gap tasks improved** (googlelocal:2 0→2/3, googlelocal:3 2/5→3/3, music_brainz:3 & yelp:7 →3/3), patents:2 unchanged; **3/3 parity tasks held**. **DO NOT merge without a full 270-run confirming net-positive** — the taxonomy lever went net-negative from exactly this shape, and the smoke tasks are the derived-from set. **CONTAMINATION LESSON:** a lever example (`5–11PM`) was a literal held-out GT token (in `ground_truth.csv`+`validate.py`); Fable review caught it — **always grep any lever example against `~/repos/DataAgentBench` GT before shipping.** xhigh runner ready (`runs/dab/run_full12_sonnet5_xhigh.sh`, `--agent-timeout 1800` since high already brushed the 1200s wall), unlaunched; **pending decision: clean high-effort lever A/B vs fold levers into xhigh.** Memory `project_dab_gap_levers`.
