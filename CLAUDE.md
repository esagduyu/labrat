...
**Phase 5 — full 54-query DAB (2026-06-01): submitted 58.0%, CORRECTED to 50.5% after a contamination audit (2026-06-03).** All 12 official datasets, claude-mcp driver, pass@5, claude-sonnet-4-6. Run dir: `runs/dab/dab-1780210698/`. Full write-up: `docs/dab-progress-report.md`.

⚠️ **The 58.0% was contaminated — do not cite it.** The `claude-mcp` driver ran `claude --print … --permission-mode bypassPermissions` with **no `--allowedTools`/`--disallowedTools`**, so the agent kept the full Claude Code toolset (Bash, WebFetch, subagents) on top of the MCP server, and the DataAgentBench checkout (every `validate.py` + `ground_truth.csv`) was readable on disk. Traces show the agent reading `ground_truth.csv`/`validate.py` and loading `load_dataset("ag_news")`. 18 of 270 trials were contaminated — **16 of 20 agnews** (95% → 15%), one bookreview (93%→87%), one yelp (63%→60%); the other nine datasets are clean. Withdrawing those 18 passes recomputes the stratified mean **58.0% → 50.5%** (local recompute reproduces 58.0% exactly). Disclosed to maintainers on PR #54 with a full 270-trial trace bundle. See `runs/dab/dab-1780210698/trace_bundle/CONTAMINATION_AUDIT.md` and the memory note `project_dab_contamination`. Surviving per-dataset signal: **crmarenapro 82% on 6 databases** (strongest), stockindex 100%, stockmarket 80%, pancancer_atlas 67%; patents 0% remains the Sonnet ceiling.

**Sandbox gate — IMPLEMENTED + VALIDATED (2026-06-03).** The leakage path is now closed by construction in the `claude-mcp` driver: (1) tool allowlist (`--allowedTools mcp__labrat` + `--disallowedTools` for every native tool); (2) filesystem isolation (`cwd=<absolute scratch dir>`, repo off-path); (3) a contamination backstop — `_detect_contamination()` scans each trial's output for answer-key/external-dataset markers and withdraws any hit as `reason="contaminated:<tag>"`, which `aggregate()` excludes alongside `infra:` (so a sandbox regression can never silently inflate the score again); (4) audit-grade traces via `LABRAT_MCP_LOG_DIR`. Network egress isolation (container / `unshare -n`) stays an environment step — not portable from Python on macOS, and moot once Bash/WebFetch are blocked. Live agnews smoke (`runs/dab/dab-1780554221/`) confirms it holds: agnews:1 PASSes via 31 MCP-only calls (`load_mongo_collection`/`attach_database`/`run_sql`, no Bash/file/HF), agnews:2 honestly times out on the hard classification — contamination detector clean on both. The smoke also caught two latent bugs (now fixed + regression-tested): relative `--mcp-config` path doubling under `cwd`, and the `:memory:` federation server crash below. The clean full re-run still has to happen; this just makes it safe.

**Clean re-run — IN PROGRESS (`runs/dab/dab-rerun-clean`, launched 2026-06-04).** Full 54-query official benchmark, claude-mcp, n=5, `--agent-timeout 1200`, driven by the self-healing local loop (`scripts/dab_rerun_loop.sh` → `dab_rerun_tick.sh`, 30-min poll). **Interim findings (don't cite a final score until done):** sandbox holds (MCP-only, zero file/web leak, no contamination outside agnews); deps_dev_v1 ~30% (was 10%), github_repos 50% flat, patents 0% flat, bookreview tracking high, agnews honest ~29%. **Key finding — agnews leaks via model *memory* even under a perfect sandbox:** Sonnet recalls the public AG News id→label mapping and applies it via SQL; `_detect_contamination` catches it only when the trace *names* the dataset, so agnews is intrinsically unreliable for a pretraining-exposed model — caveat it, don't treat its number as capability (benchmark-side fix: shuffle `article_id`s, worth raising upstream). Precedent: **DAB PR #53 (Altimate, GPT‑5.5)** hit the identical agnews `load_dataset` leak, sandboxed + re-ran, and was **accepted onto the leaderboard** (agnews 100%→35%, stratified 68.93%→63.18%) — so the contamination is model-agnostic and our disclose→sandbox→rerun path is the accepted one. See memory `project_dab_contamination` + `reference_dab_pr53_altimate_precedent`.
...
**`claude-mcp` driver traces — server-side tool-call logging now shipped (commit `6b4d3bf`).** `--output-format json` still gives one bundled CLI result (per-trial `passed` / `tool_calls` count / `latency` / `final_text`, not per-call SQL from the CLI side). But the MCP server now logs every dispatch when `LABRAT_MCP_LOG_DIR` is set — one `{"tool","input","output","ok","latency_ms"}` line per call to `<LABRAT_MCP_LOG_DIR>/mcp_tool_calls.jsonl` (`_log_tool_call` in `src/labrat/mcp/server.py`), and the driver points it at the trial scratch dir. So we get real per-tool-call audit traces for free now. Still-open enhancement if the LLM message stream is needed: switch `_run_trial_claude_mcp` to `--output-format stream-json` and persist to `scratch/<task>__trial<n>/claude_stream.jsonl`.
