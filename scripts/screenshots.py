...
async def m01_branding() -> None:
    ...
    def populate(log: RichLog) -> None:
        ...
        log.write("")
        log.write(
            "[dim]Override: drop src/labrat/assets/banner_splash.txt to swap art at zero-code-change.[/dim]"
        )
...
async def m02_database_layer() -> None:
    ...
    def populate(log: RichLog) -> None:
        ...
        log.write("")
        log.write(
            "[bold cyan]conn.execute('SELECT id, user_id, total_amount, status FROM orders LIMIT 5')[/bold cyan]"
        )
...
async def m03_profiles() -> None:
    ...
    def populate(log: RichLog) -> None:
        log.write(
            "[bold cyan]labrat conn add --name local-duck --dialect duckdb --path …[/bold cyan]"
        )
        log.write("[bold cyan]labrat conn list[/bold cyan]")
        t = Table(show_header=True, header_style="bold magenta", border_style="dim")
        t.add_column("name")
        t.add_column("dialect")
        t.add_column("read_only")
        t.add_column("default_schema")
        ...
        log.write(
            Panel(
                Syntax(
                    next(iter(loaded.values())).model_dump_json(indent=2), "json", theme="monokai"
                ),
...
async def m08_completion() -> None:
    ...
    def populate(log: RichLog) -> None:
        log.write(
            "[bold cyan]SQLCompleter.complete('SELECT * FROM ord', cursor_offset=17)[/bold cyan]"
        )
        t = Table(show_header=True, header_style="bold magenta", border_style="dim")
        ...
        log.write("")
        log.write(
            "[bold cyan]SQLCompleter.complete('SELECT orders.', cursor_offset=14)[/bold cyan]"
        )
...
async def m09_validation() -> None:
    ...
    cases = [
        ("duckdb", "SELECT id, status FROM orders WHERE status = 'pending'", "valid DuckDB SELECT"),
        ("duckdb", "SELEKT id FROM orders", "syntax error"),
        (
            "redshift",
            "SELECT PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY total_amount) FROM orders",
            "dialect error on Redshift",
        ),
        ("duckdb", "DROP TABLE orders", "mutation — DDL"),
    ]
    ...
    await _snap(
        _Demo("M9 · SQL Validation + Inline Errors via SQLGlot", populate), "m09_validation"
    )
...
async def m12_schema_tools() -> None:
    ...
    stats_r = await registry.dispatch(
        "column_stats", {"table": "orders", "column": "total_amount"}, ctx
    )
    conn.disconnect()
...
async def m13_safety() -> None:
    ...
    ok_r = await registry.dispatch(
        "run_sql", {"query": "SELECT user_id, SUM(total_amount) FROM orders GROUP BY user_id"}, ctx
    )
    exp_r = await registry.dispatch(
        "explain_sql", {"query": "SELECT * FROM orders WHERE status = 'pending'"}, ctx
    )
    conn.disconnect()
    ...
    def populate(log: RichLog) -> None:
        ...
        log.write("")
        log.write(
            "[bold cyan]run_sql('SELECT user_id, SUM(total_amount) FROM orders GROUP BY user_id')[/bold cyan]"
        )
        val = ok_r.value
        log.write(
            f"  ok={val.ok}   row_count={val.row_count}   [green]✓ auto-limit applied[/green]"
        )
...
async def m14_agent_loop() -> None:
    ...
    class _Scripted(ModelProvider):
        _turns = [
            ...
            [
                ToolUseBlock(
                    id="t2", name="run_sql", input={"query": "SELECT COUNT(*) AS cnt FROM orders"}
                )
            ],
    ...
    def populate(log: RichLog) -> None:
        ...
        log.write("")
        log.write(
            f"[dim]History turns: {len(loop.history)}  |  Tool round-trips: {len(calls)}[/dim]"
        )
...
async def m14_5_providers() -> None:
    providers = [
        ...
        (
            ...
            "gpt-4o / llama3 / …",
            "OpenAI-compatible: Azure, Ollama, LiteLLM",
        ),
...
async def m15_prompts() -> None:
    ...
    def populate(log: RichLog) -> None:
        log.write(
            f"[bold cyan]build_system_prompt(dialect) — {len(dialect_files)} dialect files[/bold cyan]"
        )
        log.write(
            f"  [dim]{', '.join(f.stem.replace('dialect_', '') for f in dialect_files)}[/dim]"
        )
        log.write("")
...
async def m17_threads() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        mgr.append_version(
            t1.id, sql="SELECT user_id, SUM(total_amount) FROM orders GROUP BY 1", chat_history=[]
        )
        t2 = mgr.create_thread(name="user-segmentation", profile_name="local-duck")
        mgr.append_version(
            t2.id, sql="SELECT region_id, COUNT(*) FROM users GROUP BY 1", chat_history=[]
        )
    ...
    def populate(log: RichLog) -> None:
        log.write(
            "[bold cyan]ThreadManager — threads as branches with versioned SQL history[/bold cyan]"
        )
        log.write("")
        t = Table(show_header=True, header_style="bold magenta", border_style="dim")
        t.add_column("thread")
        ...
        for th in threads:
            t.add_row(
                th.name, str(len(thread_versions[th.id])), th.created_at.strftime("%Y-%m-%d %H:%M")
            )
        log.write(t)
        log.write("")
        if loaded:
            log.write(
                f"[bold cyan]Thread '{loaded.name}' — {len(loaded_versions)} versions[/bold cyan]"
            )
...
async def m19_audit_log() -> None:
    ...
    def populate(log: RichLog) -> None:
        ...
        log.write(
            f"  [dim]~/.local/share/labrat/sessions/sess-001.jsonl  ({len(events)} events)[/dim]"
        )
        log.write("")
        for ev in events:
...
async def m21_chart_spec() -> None:
    ...
    class _Host(App[None]):
        ...
        def compose(self) -> ComposeResult:
            yield Static(
                "  LabRat  ·  M21 · Chart Spec + Unicode Rendering (plotext bar chart)", id="banner"
            )
...
async def m22_chart_image() -> None:
    ...
    spec = ChartSpec(
        chart_type=ChartType.line, x="order_date", y="daily_revenue", title="Daily Revenue Trend"
    )
    ...
    def populate(log: RichLog) -> None:
        ...
        t.add_column("terminal support")
        for proto, support in protocols:
            t.add_row(proto, support)
        log.write(t)
        log.write("")
        log.write(
            "[bold cyan]render_unicode(ChartSpec(line, x='order_date', y='daily_revenue'), df)[/bold cyan]"
        )
    ...
    def populate(log: RichLog) -> None:
        ...
        log.write(
            Panel(
                ...
                title="PostgresConnection capabilities",
                border_style="cyan",
            )
        )
...
async def m24_multi_connection() -> None:
    ...
    def populate(log: RichLog) -> None:
        log.write(
            "[bold cyan]ProfileManager — multi-profile CRUD + active-connection tracking[/bold cyan]"
        )
        log.write("")
        log.write(
            Syntax(
...
async def m25_warehouse_adapters() -> None:
    def populate(log: RichLog) -> None:
        ...
        log.write("")
        log.write(
            "[dim]All 5 adapters: same introspect_catalog / execute / explain / sample_table.[/dim]"
        )
...
async def m26_eval_framework() -> None:
    ...
    class _Suite:
        ...
        cases = [
            EvalCase(
                id="q1", question="How many orders?", expected_sql="SELECT COUNT(*) FROM orders"
            ),
            EvalCase(
                id="q2",
                question="Top users by revenue?",
                expected_sql="SELECT user_id, SUM(total_amount) FROM orders GROUP BY 1 ORDER BY 2 DESC LIMIT 3",
            ),
            EvalCase(
                id="q3",
                question="Orders by status?",
                expected_sql="SELECT status, COUNT(*) FROM orders GROUP BY 1",
            ),
        ]
...
async def m27_comparison() -> None:
    ...
    def populate(log: RichLog) -> None:
        ...
        for b, c in zip(baseline_results, labrat_results, strict=False):
            ...
            delta = (
                "[green]+1[/green]"
                if c.status == EvalStatus.correct and b.status != EvalStatus.correct
                else "[red]-1[/red]"
                if b.status == EvalStatus.correct and c.status != EvalStatus.correct
                else "—"
            )
        ...
        log.write(
            f"  Δ improvement:    [bold green]+{(labrat_report.accuracy - baseline_report.accuracy):.0%}[/bold green]"
        )
...
async def m28_query_history() -> None:
    ...
    def populate(rlog: RichLog) -> None:
        rlog.write(
            "[bold cyan]QueryHistoryLog — always-on PII-redacted JSONL per profile[/bold cyan]"
        )
        rlog.write(
            f"  [dim]~/.local/share/labrat/history/local-duck.jsonl  ({len(events)} events)[/dim]"
        )
        rlog.write("")
        t = Table(show_header=True, header_style="bold magenta", border_style="dim")
        ...
        for ev in events:
            t.add_row(
                ...
                ev.sql_final[:53],
                str(ev.row_count),
                str(ev.execution_time_ms),
            )
        rlog.write(t)
        rlog.write("")
        matching = [e for e in events if "orders" in e.sql_final]
        rlog.write(
            f"[bold cyan]search_query_history('orders')[/bold cyan]  → [green]{len(matching)}[/green] matching"
        )
...
async def m29_context_engine() -> None:
    ...
    descriptions = await analyzer.generate_descriptions(tables=["orders", "users"], events=events)

    def populate(log: RichLog) -> None:
        log.write(
            "[bold cyan]score_table_relevance(events) — frequency × recency weight[/bold cyan]"
        )
        t = Table(show_header=True, header_style="bold magenta", border_style="dim")
        t.add_column("table")
        t.add_column("relevance score")
    ...
    await _snap(
        _Demo("M29 · Context Engine — Personal Domain Builder", populate), "m29_context_engine"
    )
...
async def m30_external_catalog() -> None:
    ...
    await _snap(
        _Demo("M30 · External Catalog Integration (dbt + MCP)", populate), "m30_external_catalog"
    )
...
async def m31_memory() -> None:
    ...
    def populate(log: RichLog) -> None:
        log.write(
            "[bold cyan]MemoryStore — self-healing corrections, JSONL per profile[/bold cyan]"
        )
        log.write(
            f"  [dim]~/.local/share/labrat/memories/local-duck.jsonl  ({len(memories)} entries)[/dim]"
        )
        log.write("")
        t = Table(show_header=True, header_style="bold magenta", border_style="dim")
        t.add_column("scope")
        t.add_column("table")
        t.add_column("kind")
        t.add_column("text", max_width=50)
        ...
        log.write("")
        log.write(
            "[bold cyan]recall_memories(context='querying orders') → relevant entries[/bold cyan]"
        )
        log.write(
            "  [green]▸[/green] [table:orders] Valid statuses: 'pending', 'completed', 'cancelled'"
        )
...
async def m32_validations() -> None:
    ...
    for sql, summary in sql_cases:
        try:
            r = asyncio.get_event_loop().run_until_complete(checker.check(sql, summary, rules))
        except RuntimeError:
