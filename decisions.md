...
## 2026-06-22 — Cartographer DAB pre-pass (GT-firewalled AutoContext)
...
- **Next (non-negotiable):** run the §6 **ablation** — tuning subset with vs without `--agent-autocontext` + the 9-task ADE smoke — and keep AutoContext for the full run **only if net-positive** (Altimate ~+8pp; confirm on ours).
- **Deferred (minor):** clean up `self._scent_cache_root` temp dir (an `atexit`/`shutil.rmtree(ignore_errors=True)`); orphan `mkdtemp` trees accumulate across autocontext runs (benchmark-only, no answer content).

**Ablation result (2026-06-22, claude-mcp/Sonnet, n=3, tuning subset):** OFF 5/24 (21%) → ON 7/24 (29%) = **+8.3pp** (deps_dev_v1 0→33%, music_brainz_20k 0→11%, stockindex 56→44% [−1 trial, noise; deterministic Scent has structure but not the dirty-date Gotcha, which needs the off-for-DAB LLM pass]). **Decision: KEEP `--agent-autocontext` for the full run** (matches Altimate's ~+8pp). Disclose in the submission. Next: layer the cheap prompt levers (ablated) → fresh full run.
