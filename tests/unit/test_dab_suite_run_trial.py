...
from labrat.eval.benchmarks.dab.suite import DabSuite, _build_codex_mcp_prompt
from labrat.mcp.policy import McpPolicy, validate_policy_integrity
...
def test_build_codex_mcp_prompt_is_path_free_and_uses_exact_core_workflow(
    tmp_path: Path,
) -> None:
    _make_real_duckdb_fixture(tmp_path)
    task = next(iter(DabSuite(dab_dir=tmp_path).tasks()))
    ...
    for tool in _CODEX_CORE_TOOLS:
        assert tool in prompt
    assert task.prompt in prompt
    assert "Budget: at most 17 tool calls" in prompt
    assert "never answer from prior" in prompt
    assert str(tmp_path) not in prompt
    assert ".duckdb" not in prompt
    assert "db_path" not in prompt
    assert "host=" not in prompt
    assert "attach_database" not in prompt
    assert "load_mongo_collection" not in prompt


@pytest.mark.parametrize("tool_calls", [0, 2])
async def test_codex_mcp_integrates_core_policy_read_only_db_artifacts_and_usage(
    tmp_path: Path,
    monkeypatch: Any,
    tool_calls: int,
) -> None:
    ...
    suite = DabSuite(
        ...
        agent_reasoning="max",
        agent_ledger=False,
        agent_max_tool_calls=17,
    )
    task = next(iter(suite.tasks()))
    source_home = tmp_path / "source-codex-home"
    source_home.mkdir()
    (source_home / "auth.json").write_text('{"auth":true}', encoding="utf-8")
    (source_home / "config.toml").write_text("private=true", encoding="utf-8")
    monkeypatch.setenv("CODEX_HOME", str(source_home))
    monkeypatch.setattr(
        "labrat.eval.benchmarks.dab.suite.shutil.which", lambda name: "/usr/bin/codex"
    )
    captured: dict[str, Any] = {}

    async def fake_run_codex(prompt: str, config: Any, **kwargs: Any) -> NativeRunResult:
        captured.update(prompt=prompt, config=config, kwargs=kwargs)
        assert config.workspace_dir.parent == config.artifact_dir.parent
        assert config.workspace_dir.is_dir() and not any(config.workspace_dir.iterdir())
        assert config.artifact_dir.is_dir() and not any(config.artifact_dir.iterdir())
        assert config.codex_home.is_dir() and not any(config.codex_home.iterdir())
        assert config.codex_home != config.source_codex_home
        payloads = {
            "codex_events.jsonl": "events\n",
            "final_answer.txt": "3",
            "codex_token_usage.jsonl": "usage\n",
            "mcp_tool_calls.jsonl": "trace\n" if tool_calls else "",
            "private-debug.json": "must not promote",
        }
        for name, value in payloads.items():
            (config.artifact_dir / name).write_text(value, encoding="utf-8")
        return NativeRunResult(
            ...
            request_usage=(RequestUsage(1, 300, 240, 60, 20, 7),),
            mcp_calls=(),
        )

    monkeypatch.setattr("labrat.eval.benchmarks.dab.suite.run_codex", fake_run_codex)
    scratch = tmp_path / f"codex-scratch-{tool_calls}"
    result = await suite.run_trial(task, trial_num=0, scratch_dir=scratch)
    ...
    assert captured["config"].source_codex_home == source_home
    launch = captured["config"].mcp
    assert launch.command == __import__("sys").executable
    assert launch.args == ("-m", "labrat.mcp.server")
    assert launch.enabled_tools == _CODEX_CORE_TOOLS
    server_env = dict(launch.env)
    assert set(server_env) == {
        "LABRAT_MCP_CONNECTIONS",
        "LABRAT_MCP_PRIMARY",
        "LABRAT_MCP_LOG_DIR",
        "LABRAT_MCP_POLICY_PATH",
    }
    connections = json.loads(server_env["LABRAT_MCP_CONNECTIONS"])
    assert connections == {
        "main_database": {
            "db_type": "duckdb",
    ...
    policy = McpPolicy.model_validate_json(
        Path(server_env["LABRAT_MCP_POLICY_PATH"]).read_text(encoding="utf-8")
    )
    validate_policy_integrity(policy)
    assert policy.allowed_tools == _CODEX_CORE_TOOLS
    assert policy.source_grants == ()
    assert policy.mongo_grants == ()
    assert policy.primary_database == "main_database"
    assert policy.cartographer_enabled is False
    assert len(policy.run_manifest_sha256) == len(policy.builder_sha256) == 64
    for name, expected in {
        ...
        assert (scratch / name).read_text(encoding="utf-8") == expected
    assert not (scratch / "private-debug.json").exists()
...
@pytest.mark.parametrize(
    ...
    [
        ({}, 1),
        ({"agent_reasoning": None}, 0),
        ({"agent_ledger": True}, 0),
        ({"agent_verify": True}, 0),
        ({"agent_max_turns": 2}, 0),
        ({"cartograph": True}, 0),
        ({"cartograph_semantics": True}, 0),
        ({"consensus_k": 2}, 0),
        ({"reverify": True}, 0),
        ({"argue_rounds": 1}, 0),
        ({"postverify": True}, 0),
...
async def test_codex_mcp_rejects_non_diagnostic_options_before_inference(
    tmp_path: Path,
    monkeypatch: Any,
    ...
    suite = DabSuite(**kwargs)
    task = next(iter(suite.tasks()))
    run = AsyncMock()
    monkeypatch.setattr("labrat.eval.benchmarks.dab.suite.run_codex", run)

    result = await suite.run_trial(task, trial_num=trial_num, scratch_dir=tmp_path / "scratch")

    assert result.passed is False
    assert result.reason == "audit-error"
    run.assert_not_awaited()


@pytest.mark.parametrize("shape", ["attachable", "mongo", "secondary_duckdb"])
async def test_codex_mcp_rejects_non_single_duckdb_environments(
    tmp_path: Path, monkeypatch: Any, shape: str
) -> None:
    if shape == "attachable":
        _make_mixed_db_fixture(tmp_path)
    else:
        _make_real_duckdb_fixture(tmp_path)
        config_path = tmp_path / "query_real1" / "db_config.yaml"
        config = yaml.safe_load(config_path.read_text())
        if shape == "mongo":
            config["db_clients"]["events"] = {"db_type": "mongo", "db_name": "events"}
        else:
            other = tmp_path / "other.duckdb"
            other.touch()
            config["db_clients"]["other"] = {
                "db_type": "duckdb",
                "db_path": str(other),
            }
...
async def test_codex_mcp_audit_failure_is_sanitized_before_validator(
    tmp_path: Path, monkeypatch: Any
) -> None:
    _make_real_duckdb_fixture(tmp_path)
    suite = DabSuite(
    ...
    task = next(iter(suite.tasks()))
    monkeypatch.setattr(
        "labrat.eval.benchmarks.dab.suite.run_codex",
        AsyncMock(side_effect=CodexAuditError("secret SQL SELECT private")),
    )
    validator = MagicMock(return_value=(True, "must not run"))
    monkeypatch.setattr("labrat.eval.benchmarks.dab.scorer.score_with_validator", validator)
    monkeypatch.setattr(
        "labrat.eval.benchmarks.dab.suite.shutil.which", lambda name: "/usr/bin/codex"
    )
    ...
    assert result.reason == "audit-error"
    assert result.artifact == {"type": "text", "payload": "Native Codex audit failed"}
    assert "secret" not in json.dumps(result.artifact)
    validator.assert_not_called()


async def test_codex_mcp_rate_limit_preserves_sanitized_infra_meta(
    tmp_path: Path, monkeypatch: Any
) -> None:
    _make_real_duckdb_fixture(tmp_path)
    suite = DabSuite(
    ...
    monkeypatch.setattr(
        "labrat.eval.benchmarks.dab.suite.run_codex",
        AsyncMock(
            side_effect=CodexInfrastructureError(
                "rate_limit", meta={"exit_code": 7, "resets_at": 123}
            )
        ),
    )
    validator = MagicMock(return_value=(True, "must not run"))
    ...
    assert result.reason == "infra:rate_limit"
    assert result.meta["infrastructure"] == {"exit_code": 7, "resets_at": 123}
    assert result.artifact == {"type": "text", "payload": "Native Codex infrastructure failed"}
    validator.assert_not_called()
