...
def test_codex_mcp_defaults_to_single_trial_luna_max_diagnostic_config(
    tmp_path: Path,
) -> None:
    from scripts.eval_dab import main
    ...
    dab_dir = tmp_path / "empty-dab"
    dab_dir.mkdir()
    output_dir = tmp_path / "codex-diagnostic"
    ...
    config = json.loads((output_dir / "config.json").read_text())
    assert config["driver"] == "codex-mcp"
    assert config["agent_provider"] == "codex"
    assert config["agent_model"] == "gpt-5.6-luna"
    assert config["agent_reasoning"] == "max"
    assert config["n_trials"] == 1
    assert config["agent_ledger"] is False
    assert config["tool_profile"] == "dab-core-v1"
    assert config["submission_eligibility"] == "diagnostic-only"
    assert config["trace_contract"] == "native-reconciled"
    assert not (output_dir / "submission.json").exists()


def test_codex_mcp_is_documented_as_diagnostic_only(
    capsys: pytest.CaptureFixture[str],
) -> None:
    from scripts.eval_dab import main
...
@pytest.mark.parametrize(
    ("extra", "flag_name"),
    [
        (["--agent-provider", "anthropic"], "agent-provider"),
        (["--n-trials", "2"], "n-trials"),
        (["--agent-ledger"], "agent-ledger"),
        (["--agent-verify"], "agent-verify"),
        (["--agent-cartograph"], "agent-cartograph"),
        (["--agent-cartograph", "--cartograph-semantics"], "cartograph"),
        (["--agent-consensus", "2"], "agent-consensus"),
        (["--agent-reverify"], "agent-reverify"),
        (["--agent-argue-rounds", "1"], "agent-argue-rounds"),
        (["--agent-postverify"], "agent-postverify"),
        (["--max-turns", "2"], "max-turns"),
    ],
)
def test_codex_mcp_rejects_unsupported_options_before_suite_construction(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    extra: list[str],
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: (_ for _ in ()).throw(
            AssertionError("invalid codex-mcp config must fail before suite construction")
        ),
    )
    dab_dir = tmp_path / "empty-dab"
    dab_dir.mkdir()
...
@pytest.mark.parametrize(
    "extra",
    [
        ["--agent-model", "gpt-5.6-moon"],
        ["--agent-model", "gpt-5.6-luna", "--agent-reasoning", "ultra"],
    ],
)
def test_codex_mcp_preserves_native_model_effort_validation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    extra: list[str],
) -> None:
    ...
    monkeypatch.setattr(
        ...
        "DabSuite",
        lambda **_kwargs: (_ for _ in ()).throw(
            AssertionError("invalid model config must fail before suite construction")
        ),
    )
    dab_dir = tmp_path / "empty-dab"
    dab_dir.mkdir()
...
@pytest.mark.parametrize(
    ("field", "value", "flag_name"),
    [
        ("agent_ledger", True, "agent-ledger"),
        ("agent_verify", True, "agent-verify"),
        ("agent_max_turns", 10, "max-turns"),
    ],
)
def test_codex_mcp_rejects_unsupported_restored_config(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    field: str,
    ...
    monkeypatch.setattr(
        ...
        "DabSuite",
        lambda **_kwargs: (_ for _ in ()).throw(
            AssertionError("unsupported resume config must fail before suite construction")
        ),
    )

    with pytest.raises(SystemExit) as raised:
        eval_dab.main(["--dab-dir", str(dab_dir), "--output-dir", str(output_dir)])

    assert raised.value.code == 2
    assert flag_name in capsys.readouterr().err


def test_codex_mcp_resume_rejects_provider_and_cardinality_conflicts(tmp_path: Path) -> None:
    from scripts.eval_dab import main
    ...
    with pytest.raises(SystemExit, match=r"Resume conflict.*agent-provider"):
        main([*base, "--agent-provider", "anthropic"])
    with pytest.raises(SystemExit, match=r"Resume conflict.*n-trials"):
        main([*base, "--n-trials", "2"])


def test_codex_mcp_rejects_stale_submission_before_trial_call(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    class NoRunSuite:
        ...
        async def run_trial(
            ...
            nonlocal calls
            calls += 1
            raise AssertionError("stale diagnostic output must fail before execution")

    monkeypatch.setattr(eval_dab, "DabSuite", lambda **_kwargs: NoRunSuite())
    dab_dir = tmp_path / "empty-dab"
    dab_dir.mkdir()
    output_dir = tmp_path / "stale-codex"
    output_dir.mkdir()
    stale = b'[{"answer":"historical"}]\n'
    (output_dir / "submission.json").write_bytes(stale)
...
def test_codex_mcp_success_is_taint_audited_reported_and_never_submitted(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from scripts import eval_dab
    ...
    class DiagnosticSuite:
        ...
        def write_submission(self, *_args: object, **_kwargs: object) -> None:
            raise AssertionError("diagnostic codex-mcp must never write submission.json")

    suite_kwargs: dict[str, object] = {}

    def build_suite(**kwargs: object) -> DiagnosticSuite:
        suite_kwargs.update(kwargs)
        return DiagnosticSuite()

    audits: list[tuple[Path, Path]] = []

    def fake_audit(trials_jsonl: Path, scratch_dir: Path) -> dict[str, str]:
        audits.append((trials_jsonl, scratch_dir))
        return {"ds:1:0": "clean"}

    monkeypatch.setattr(eval_dab, "DabSuite", build_suite)
    monkeypatch.setattr(eval_dab, "audit_run", fake_audit)
    dab_dir = tmp_path / "empty-dab"
    dab_dir.mkdir()
    ...
    assert suite_kwargs["driver"] == "codex-mcp"
    assert suite_kwargs["agent_provider"] == "codex"
    assert suite_kwargs["agent_model"] == "gpt-5.6-luna"
    assert suite_kwargs["agent_reasoning"] == "max"
    assert suite_kwargs["agent_ledger"] is False
    assert audits == [(output_dir / "trials.jsonl", output_dir / "scratch")]
    assert not (output_dir / "submission.json").exists()
    report = (output_dir / "report.md").read_text()
    assert "Diagnostic-only" in report
    assert "Submission eligibility: **ineligible**" in report


def test_codex_mcp_audit_error_appends_then_stops_and_remains_retryable(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    ...
    class AuditErrorSuite:
        ...
        async def run_trial(
            ...
            return TrialResult(
                ...
                reason="audit-error",
                latency_seconds=0.0,
                artifact={"type": "text", "payload": "sanitized audit failure"},
            )

        def aggregate(self, _results: list[TrialResult]) -> AggregateScore:
            raise AssertionError("audit stop must occur before aggregation")

    suite = AuditErrorSuite()
    monkeypatch.setattr(eval_dab, "DabSuite", lambda **_kwargs: suite)
    monkeypatch.setattr(
        eval_dab,
        "audit_run",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("audit stop must occur before taint packaging")
        ),
    )
    dab_dir = tmp_path / "empty-dab"
    dab_dir.mkdir()
    output_dir = tmp_path / "audit-stop"
    base = [
        "--dab-dir",
        str(dab_dir),
        "--output-dir",
    ...
    assert eval_dab.main(base) == eval_dab._AUDIT_EXIT_CODE == 5
    assert suite.calls == 1
    rows = [json.loads(line) for line in (output_dir / "trials.jsonl").read_text().splitlines()]
    assert len(rows) == 1
    assert rows[0]["reason"] == "audit-error"
    assert ("ds:1", 0) not in eval_dab._load_completed_trials(output_dir / "trials.jsonl")
    assert not (output_dir / "taint.json").exists()
    assert not (output_dir / "submission.json").exists()
    assert not (output_dir / "report.md").exists()
    ...
    # Resume retries the same key, appends its new audit row, and stops again
    # before advancing to ds:2.
    assert eval_dab.main(base) == eval_dab._AUDIT_EXIT_CODE
    assert suite.calls == 2
    assert len((output_dir / "trials.jsonl").read_text().splitlines()) == 2
